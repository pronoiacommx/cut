...
class CompileCatalog(CommandMixin):
    description = 'compile message catalogos to binary MO files'
    user_options = [
        ('domain=', 'D',
         "domains of PO files (space separated list, default 'messages')"),
        ('directory=', 'd',
         'path to base directory containing the catalogos'),
        ('input-file=', 'i',
    ...
    def _run_domain(self, domain):
        ...
        if not po_files:
            raise OptionError('no message catalogos found')

        catalogos_and_errors = {}
        ...
        for idx, (locale, po_file) in enumerate(po_files):
            ...
            catalogos_and_errors[catalog] = catalog_errors = list(catalog.check())
...
class UpdateCatalog(CommandMixin):
    description = 'update message catalogos from a POT file'
    user_options = [
        ('domain=', 'D',
         "domain of PO file (default 'messages')"),
        ('input-file=', 'i',
    ...
    def run(self):
        ...
        if self.check:
            ...
            if not all(check_status.values()):
                raise BaseError("Some catalogos are out of date.")
            else:
                self.log.info("All the catalogos are up-to-date.")
            return
...
class CommandLineInterface:
    ...
    commands = {
        'compile': 'compile message catalogos to MO files',
        'extract': 'extract messages from source files and generate a POT file',
        'init': 'create new message catalogos from a POT file',
        'update': 'update existing message catalogos from a POT file',
    }
